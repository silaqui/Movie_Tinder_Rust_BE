...
#[get("/start")]
fn start(user_id: UserId, session_manager: &State<Arc<Mutex<SessionManager>>>) -> Json<NextMovie> {
    log::info!("Start session - user: {:?}", user_id);
    Json(session::start(&user_id, &session_manager))
}

#[get("/join/<session_id>")]
fn join(user_id: UserId, session_manager: &State<Arc<Mutex<SessionManager>>>, session_id: String) -> Json<NextMovie> {
...
fn vote(user_id: UserId, session_manager: &State<Arc<Mutex<SessionManager>>>, session_id: String, vote: Json<Vote>) -> Json<NextMovie> {
