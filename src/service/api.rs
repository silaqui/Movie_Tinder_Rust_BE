...
use crate::service::model::{HitCount, Movie, Vote};
...
fn start(cookies: &CookieJar<'_>, hit_count: &State<HitCount>) -> Json<Movie> {
    ...
    Json(session::start(user_id).movie)
}

#[get("/join/<session_id>")]
fn join(cookies: &CookieJar<'_>, hit_count: &State<HitCount>, session_id: String) -> Json<Movie> {

    let user_id = user::identify_user(cookies, hit_count);

    log::info!("Join session - user: {:?} - session: {}", user_id, session_id);

    Json(session::join(user_id, session_id).movie)
}

#[post("/join/<session_id>" , format = "json", data = "<vote>" )]
fn vote(cookies: &CookieJar<'_>, hit_count: &State<HitCount>, session_id: String, vote: Json<Vote>) -> Json<Movie> {
    ...
    Json(session::vote(user_id, session_id, vote).movie)
}
